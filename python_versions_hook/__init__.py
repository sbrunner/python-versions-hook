...
def _detect_python_version(directory: Path) -> packaging.specifiers.SpecifierSet | packaging.version.Version | None:
    """Detect Python version for a directory with priority:
...
def _get_python_version(directory: Path) -> tuple[
...
def main() -> None:
    ...
    for directory in _get_all_directories():
        ...
        _update_files_in_directory(directory, minimal_version, first_version, last_version, version)
...
def _update_files_in_directory(
    ...
    version: packaging.specifiers.SpecifierSet | packaging.version.Version,
) -> None:
